...
class Songlistrecommendation extends Component {
   constructor(props){
    ...
    Right(){
      ...
      console.log(document.getElementsByClassName('utf8s').length)
                ...
                <div className="qqslide">
                  {
                    listnoe.length > 0 ? listnoe.map((item,index)=>{
                      return  <dl key={index} className="utf8s">
                          <dt>{listnoe ? <img className="coverimg" src={item.cover || item.cover_url_big} alt=""/> : <img className="coverimg" src={this.state.lat} alt=""/>}<div className='coverimg' ><div  className='coverimgs' onClick={()=>{this.imgsone(item,index)}} style={{backgroundImage: 'url(' + cover + ')'}}></div> </div></dt>
                          <dd>
                            <p >{item.title}</p>
